...
@RestController
@RequestMapping("api/prod/patients")
@Profile("prod")
public class PatientProdController {

    private final PatientService patientService;

    @Autowired
    public PatientProdController(PatientService patientService) {
    ...
    public ResponseEntity<EntityModel<PatientDetailedDTO>> create(@RequestBody @Valid PatientRegisterDTO patientRegisterDTO,
        ...
        Link appointmentsLink = linkTo(methodOn(PatientDevController.class).list(Pageable.unpaged())).withRel("all-patients");
        patientModel.add(selfLink, appointmentsLink);

        URI location = uriBuilder.path("/api/prod/patients/{id}").buildAndExpand(patient.id()).toUri();
