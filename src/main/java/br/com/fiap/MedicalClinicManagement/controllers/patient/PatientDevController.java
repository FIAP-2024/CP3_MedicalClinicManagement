...
@RestController
@RequestMapping("api/dev/patients")
@Profile("dev")
public class PatientDevController {

    private final PatientService patientService;

    @Autowired
    public PatientDevController(PatientService patientService) {
    ...
    public ResponseEntity<EntityModel<PatientDetailedDTO>> create(@RequestBody @Valid PatientRegisterDTO patientRegisterDTO,
        ...
        URI location = uriBuilder.path("/api/dev/patients/{id}").buildAndExpand(patient.id()).toUri();
